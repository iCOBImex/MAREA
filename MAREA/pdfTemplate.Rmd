...
Para cada uno de los indicadores numericos generamos una grafica y una tabla. Las figuras presentan series de tiempo de cada indicador. Los puntos son ajustados verticalmente para mejorar la visualizacion, pero se debe de entender que los muestreos se realizan de manera anual. Cada punto representa un transecto. Las lineas negras representan el promedio anual por tratamiento (reserva / control), y las bandas de colores indican $\pm$ 1 error estandar con respecto de la media. La informacion de la reserva aparecera en rojo, y la de la(s) zona(s) control en azul. En cuanto a las tablas, presentamos las estimaciones de todos los coeficientes de las variables utilizadas en la regresion lineal. El termino **Efecto** representa el efecto que ha tenido la reserva en cada indicador. Los asteriscos indican la significancia asociada al coeficiente: \* p < 0.1, \*\* p < 0.05, \*\*\* p < 0.01.
...
```{r}
if(!is.null(gov)){
  cat("## Trabajos alternativos a pesca\n")
...
```{r}
if(!is.na(results_soc$plot[[3]])) {
  cat("## Arribos de especie objetivo 1\n")
  results_soc$plot[[3]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[3]])) {
  results_soc$model[[3]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[4]])) {
  cat("## Arribos de especie objetivo 2\n")
  results_soc$plot[[4]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[4]])) {
  results_soc$model[[4]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[5]])) {
  cat("## Arribos de especie objetivo 3\n")
  results_soc$plot[[5]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[5]])) {
  results_soc$model[[5]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[6]])) {
  cat("## Arribos de especie objetivo 4\n")
  results_soc$plot[[6]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[6]])) {
  results_soc$model[[6]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[7]])) {
  cat("## Arribos de especie objetivo 5\n")
  results_soc$plot[[7]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[7]])) {
  results_soc$model[[7]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[8]])) {
  cat("## Arribos de especie objetivo 6\n")
  results_soc$plot[[8]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[8]])) {
  results_soc$model[[8]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[9]])) {
  cat("## Arribos de especie objetivo 7\n")
  results_soc$plot[[9]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[9]])) {
  results_soc$model[[9]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
...
```{r}
if(!is.na(results_soc$plot[[10]])) {
  cat("## Arribos de especie objetivo 8\n")
  results_soc$plot[[10]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[10]])) {
  results_soc$model[[10]] %>%
    stargazer(
      dep.var.labels = "Arribos (Kg/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 1-->

```{r}
if(!is.na(results_soc$plot[[11]])) {
  cat("## Ingresos por arribos de especie objetivo 1\n")
  results_soc$plot[[11]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[11]])) {
  results_soc$model[[11]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 2-->

```{r}
if(!is.na(results_soc$plot[[12]])) {
  cat("## Ingresos por arribos de especie objetivo 2\n")
  results_soc$plot[[12]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[12]])) {
  results_soc$model[[12]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 3-->

```{r}
if(!is.na(results_soc$plot[[13]])) {
  cat("## Ingresos por arribos de especie objetivo 3\n")
  results_soc$plot[[13]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[13]])) {
  results_soc$model[[13]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 4-->

```{r}
if(!is.na(results_soc$plot[[14]])) {
  cat("## Ingresos por arribos de especie objetivo 4\n")
  results_soc$plot[[14]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[14]])) {
  results_soc$model[[14]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 5-->

```{r}
if(!is.na(results_soc$plot[[15]])) {
  cat("## Ingresos por arribos de especie objetivo 5\n")
  results_soc$plot[[15]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[15]])) {
  results_soc$model[[15]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 6-->

```{r}
if(!is.na(results_soc$plot[[16]])) {
  cat("## Ingresos por arribos de especie objetivo 6\n")
  results_soc$plot[[16]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[16]])) {
  results_soc$model[[16]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 7-->

```{r}
if(!is.na(results_soc$plot[[17]])) {
  cat("## Ingresos por arribos de especie objetivo 7\n")
  results_soc$plot[[17]]
}
```

```{r, results = 'asis'}
if(!is.na(results_soc$model[[17]])) {
  results_soc$model[[17]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
<!-- ## Ingresos por arribos sp obj 8-->

```{r}
if(!is.na(results_soc$plot[[18]])) {
  cat("## Ingresos por arribos de especie objetivo 1\n")
  results_soc$plot[[18]]
}
```
...
```{r, results = 'asis'}
if(!is.na(results_soc$model[[18]])) {
  results_soc$model[[18]] %>%
    stargazer(
      dep.var.labels = "Ingresos por arribos (pesos/ano)",
      header = F,
      type = "latex",
...
```{r}
if(any(is.na(gov$Q2_Reason))){
  cat("## Razonamiento para el diseno de la reserva\n")
  
  gov %>% 
    filter(!is.na(Q2_Reason)) %>% 
...
```{r}
if(any(is.na(gov$Q3), is.na(gov$Q4_1), is.na(gov$Q4_2), is.na(gov$Q12), is.na(gov$Q12_1))){
  cat("## Representacion\n")
